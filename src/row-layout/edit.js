...
const DEFAULT_LAYOUT_BY_COLUMNS = {
	1: '1-1',
	2: 'equal',
	3: '3-equal',
	4: '4-equal',
...
const DEFAULT_SPACING_ATTRIBUTES = {
	linkedPadding: false,
	paddingTop: 25,
	paddingRight: 0,
	paddingBottom: 25,
...
function widthsAreEqual( a = [], b = [] ) {
	...
	return a.every( ( value, index ) => Math.abs( value - b[ index ] ) < 0.1 );
}
		...
		<div className="buttercup-row-layout-columns-picker">
			{ [ 1, 2, 3, 4 ].map( ( n ) => (
				<button
					key={ n }
					type="button"
					className={ `buttercup-row-layout-columns-picker__btn${
						n === columns ? ' is-active' : ''
					}` }
					onClick={ () => onChange( n ) }
				>
	...
	return (
		<div className="buttercup-row-layout-presets">
			{ presets.map( ( { key, widths } ) => {
				const isActive = widths.every(
					( w, i ) => Math.abs( w - currentWidths[ i ] ) < 0.1
				);
				return (
					<button
						key={ key }
						type="button"
						className={ `buttercup-row-layout-presets__btn${
							isActive ? ' is-active' : ''
						}` }
		...
		<div className="buttercup-bg-type-selector">
			{ types.map( ( { key, label } ) => (
				<button
					key={ key }
					type="button"
					className={ `buttercup-bg-type-selector__btn${
						key === value ? ' is-active' : ''
					}` }
					onClick={ () => onChange( key ) }
				>
	...
	const handlePointerDown = useCallback(
		( e, index ) => {
			...
			const handlePointerMove = ( moveEvent ) => {
				...
				liveWidthsRef.current = newWidths;
				if ( onResizePreview ) {
					onResizePreview( newWidths );
				}
				setTooltip( {
					index,
					left: leftW,
	...
	return (
		<div className="buttercup-column-resizers" aria-hidden="true">
			{ handles.map( ( { index, position, gapsBefore } ) => (
				<div
					key={ index }
					className={ `buttercup-column-resizer${
						dragging === index ? ' is-dragging' : ''
					}` }
					style={ {
						left: `calc(${ position }% + ${
							gapsBefore * columnGap - columnGap / 2
						}px)`,
					} }
...
function getActiveBgType( attributes ) {
	...
	if ( attributes.backgroundVideoUrl ) {
		return 'video';
	}
...
export default function Edit( { attributes, setAttributes, clientId } ) {
	...
	const themeContentWidth = layout?.contentSize
		? parseInt( layout.contentSize, 10 )
		: 0;

	useEffect( () => {
		if ( ! uniqueId ) {
			setAttributes( {
				uniqueId: 'rl-' + Math.random().toString( 36 ).substr( 2, 6 ),
			} );
		} else {
			const duplicates = [];
			const collectRowLayouts = ( blocks ) => {
				blocks.forEach( ( block ) => {
					if (
						block.name === 'buttercup/row-layout' &&
			...
			if ( duplicates.length > 0 ) {
				setAttributes( {
					uniqueId:
						'rl-' + Math.random().toString( 36 ).substr( 2, 6 ),
				} );
			}
		}
	...
	const committedWidths = useMemo(
		() => getColumnWidths( columns, columnLayout, columnWidths ),
		[ columns, columnLayout, columnWidths ]
	);
	const [ previewWidths, setPreviewWidths ] = useState( committedWidths );
	const [ isResizingColumns, setIsResizingColumns ] = useState( false );

	useEffect( () => {
		if ( ! isResizingColumns ) {
			setPreviewWidths( committedWidths );
		}
	}, [ committedWidths, isResizingColumns ] );

	const handleColumnResizeStart = useCallback( () => {
		setIsResizingColumns( true );
	}, [] );

	const handleColumnResizePreview = useCallback( ( newWidths ) => {
		setPreviewWidths( newWidths );
	}, [] );
	...
	const handleColumnResizeCommit = useCallback(
		( newWidths ) => {
			...
			setPreviewWidths( newWidths );
			if ( widthsAreEqual( newWidths, committedWidths ) ) {
				return;
			}
			setAttributes( {
		...
		[ committedWidths, setAttributes ]
	);

	const resetColumnWidths = () => {
		const preset = DEFAULT_LAYOUT_BY_COLUMNS[ columns ] || 'equal';
		const nextWidths = getColumnWidths( columns, preset, [] );
		setIsResizingColumns( false );
		setPreviewWidths( nextWidths );
		setAttributes( { columnLayout: preset, columnWidths: [] } );
	};

	const setLinkedPaddingValue = ( value ) => {
		setAttributes( {
			paddingTop: value,
			paddingRight: value,
			paddingBottom: value,
	...
	const resetSpacingAttributes = () => {
		setAttributes( DEFAULT_SPACING_ATTRIBUTES );
	};
	...
	const widths = isResizingColumns ? previewWidths : committedWidths;
	const gapOffset =
	...
	if ( inheritMaxWidth && themeContentWidth ) {
		maxWidthHelp = `${ __(
			'Theme content width:',
			'buttercup'
		) } ${ themeContentWidth }px`;
	} else if ( inheritMaxWidth ) {
		maxWidthHelp = __(
			'No theme content width detected. Falling back to 1200px.',
			'buttercup'
		);
	}
			...
			<InspectorControls>
				...
				>
					<SelectControl
						...
						options={ [
							{
								label: __( 'Inherit Desktop', 'buttercup' ),
								value: 'inherit',
							},
							{
								label: __( 'Collapse to Rows', 'buttercup' ),
								value: 'collapse',
							},
						] }
						onChange={ ( v ) =>
							setAttributes( { tabletLayout: v } )
						}
						__nextHasNoMarginBottom
					/>
					{ tabletLayout !== 'collapse' && (
						<ToggleControl
							label={ __(
								'Reverse Column Order on Tablet',
								'buttercup'
							) }
							checked={ reverseOnTablet }
							onChange={ ( v ) =>
								setAttributes( { reverseOnTablet: v } )
							}
							__nextHasNoMarginBottom
						/>
					) }
					...
					{ mobileLayout === 'collapse' && (
						<ToggleControl
							...
							onChange={ ( v ) =>
								setAttributes( { reverseOnMobile: v } )
							}
							__nextHasNoMarginBottom
						/>
					) }
				...
				<PanelBody
					title={ __( 'Column Order', 'buttercup' ) }
					initialOpen={ false }
				>
					<p
						style={ {
					...
					>
						{ __(
							'Set per-column display order at each breakpoint. Edit each column to set its order values.',
							'buttercup'
						) }
					</p>
					{ innerBlocks.map( ( block, i ) => (
						<div
							key={ block.clientId }
							className="buttercup-column-order-group"
						>
					...
					<p style={ { fontSize: 11, color: '#999', marginTop: 8 } }>
						{ __(
							'Click a column to edit its order values directly.',
							'buttercup'
						) }
					</p>
				</PanelBody>
				...
				>
					...
					{ activeBgType === 'gradient' && (
						<GradientPicker
							...
							onChange={ ( v ) =>
								setAttributes( { backgroundGradient: v || '' } )
							}
						/>
					) }
				...
				<PanelBody
					title={ __( 'Spacing', 'buttercup' ) }
					initialOpen={ false }
				>
					<ToggleControl
						label={ __(
							'Link Desktop Padding Sides',
							'buttercup'
						) }
						checked={ !! linkedPadding }
						onChange={ ( value ) =>
							setAttributes( { linkedPadding: value } )
						}
						__nextHasNoMarginBottom
					/>
					{ linkedPadding ? (
						<RangeControl
							label={ __( 'Padding (px)', 'buttercup' ) }
							value={ paddingTop }
							onChange={ setLinkedPaddingValue }
							min={ 0 }
							max={ 200 }
							step={ 1 }
							__nextHasNoMarginBottom
					...
					>
						...
						>
							{ __(
								'Leave all at 0 to inherit desktop padding.',
								'buttercup'
							) }
						</p>
					...
					>
						...
						>
							{ __(
								'Leave all at 0 to inherit tablet/desktop padding.',
								'buttercup'
							) }
						</p>
						{ [ 'Top', 'Right', 'Bottom', 'Left' ].map(
					...
					<Button
						variant="secondary"
						onClick={ resetSpacingAttributes }
						style={ { marginTop: 16 } }
					>
						{ __( 'Reset Spacing Defaults', 'buttercup' ) }
					</Button>
				</PanelBody>

				<PanelBody
					title={ __( 'Max Width', 'buttercup' ) }
					initialOpen={ false }
				>
					<ToggleControl
						...
						onChange={ ( v ) =>
							setAttributes( { inheritMaxWidth: v } )
						}
						help={ maxWidthHelp }
						__nextHasNoMarginBottom
					/>
				...
				<PanelBody
					title={ __( 'Vertical Alignment', 'buttercup' ) }
					initialOpen={ false }
				>
					<SelectControl
						...
						options={ [
							...
							{
								label: __( 'Bottom', 'buttercup' ),
								value: 'bottom',
							},
							{
								label: __( 'Stretch', 'buttercup' ),
								value: 'stretch',
							},
						] }
						onChange={ ( v ) =>
