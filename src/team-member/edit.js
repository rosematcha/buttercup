...
export default function Edit( {
	...
	const memberPagesEnabledFromParent =
	...
	const baseUrl = permalink
		? permalink.split( '?' )[ 0 ].replace( /\/$/, '' )
		: '';
	const memberUrl =
		memberPagesEnabled && memberSlug && baseUrl
			? `${ baseUrl }/${ memberSlug }`
			: '';
	const conflictSuggestion = slugDuplicate
		? `${ memberSlug }-${ clientId.slice( 0, 4 ) }`
		: '';
	...
	useEffect( () => {
		if ( ! profileImageId || ! media ) {
			return;
		}
		if ( profileImageSource === 'square-600' ) {
			return;
		}
		const target = imageSize || 600;
	...
	return (
		<article { ...useBlockProps( { className: 'buttercup-team-member' } ) }>
			...
			<InspectorControls>
				<PanelBody
					title={ __( 'Profile Image', 'buttercup' ) }
					initialOpen={ true }
				>
					<MediaUploadCheck>
						{ profileImageUrl && (
				...
				<PanelBody
					title={ __( 'Member Details', 'buttercup' ) }
					initialOpen={ true }
				>
					<TextControl
					...
					{ ! showPronounsFromParent && (
						...
						>
							{ __(
								'Pronouns are hidden by the Team block setting.',
								'buttercup'
							) }
						</p>
					) }
					<ToggleControl
						...
						help={
							! showBioFromParent
								? __(
										'Disabled by Team block setting.',
										'buttercup'
								  )
								: undefined
						}
				...
				<PanelBody
					title={ __( 'Contact Info', 'buttercup' ) }
					initialOpen={ false }
				>
					<TextControl
				...
				<PanelBody
					title={ __( 'Member Page', 'buttercup' ) }
					initialOpen={ false }
				>
					<ToggleControl
					...
					{ memberPagesEnabled && ! memberSlug && (
						...
						>
							{ __(
								'URL slug will be generated from the name.',
								'buttercup'
							) }
						</p>
					) }
					{ memberPagesEnabled && slugDuplicate && (
						<Notice status="warning" isDismissible={ false }>
							{ __(
								'This slug is already used by another member on this page.',
								'buttercup'
							) }
						</Notice>
					) }
						...
						showSlugSettings && (
							<Button
								...
								onClick={ () =>
									setAttributes( {
										memberSlug:
											cleanForSlug( conflictSuggestion ),
									} )
								}
							>
								{ __(
									'Use Suggested Unique Slug',
									'buttercup'
								) }
							</Button>
						) }
					...
					{ memberPagesEnabled && ! memberUrl && (
						...
						>
							{ __(
								'Save this page to generate a preview URL.',
								'buttercup'
							) }
						</p>
					) }
					...
					{ memberPagesEnabled && (
						<TextareaControl
							...
							value={ longBio }
							onChange={ ( v ) =>
								setAttributes( { longBio: v } )
							}
							help={ __(
								'Used on the individual member page. If empty, the short bio is used.',
								'buttercup'
							) }
						/>
					) }
			...
			<div className="buttercup-team-member__text">
				...
				{ showPronounsFromParent && pronouns && (
					<p className="buttercup-team-member__subtitle">
						{ showPronounsFromParent && pronouns && (
							<span className="buttercup-team-member__pronouns">
								{ pronouns }
							</span>
						) }
					</p>
