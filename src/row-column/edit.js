		...
		<>
			<InspectorControls>
				...
				>
					...
					>
						{ __(
							'Set the display order for this column at each breakpoint. Lower numbers appear first. 0 = default position.',
							'buttercup'
						) }
					</p>
					<RangeControl
				...
				<PanelBody
					title={ __( 'Column Style', 'buttercup' ) }
					initialOpen={ false }
				>
					<SelectControl
					...
					/>
					<ToggleControl
						label={ __( 'Link Padding Sides', 'buttercup' ) }
						checked={ !! linkedPadding }
						onChange={ ( v ) =>
							setAttributes( { linkedPadding: v } )
						}
						__nextHasNoMarginBottom
					/>
					{ linkedPadding ? (
