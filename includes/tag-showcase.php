<?php
/**
 * Tag showcase block server rendering and helpers.
 *
 * @package Buttercup
 */
...
/**
 * Sanitize a CSS color value for safe inline style output.
 *
 * @param string $value Raw color value.
 * @return string Sanitized color string or empty string.
 */
...
/**
 * Get post types that are valid for the tag showcase block.
 *
 * @return array List of valid post type slugs.
 */
function buttercup_tag_showcase_get_valid_post_types() {
	...
	foreach ( $objects as $type => $obj ) {
		...
		if ( in_array( $type, array( 'attachment', 'revision', 'nav_menu_item', 'wp_block' ), true ) ) {
			continue;
		}
...
/**
 * Get the current post ID from context, query params, or queried object.
 *
...
/**
 * Sanitize and normalize tag showcase block attributes.
 *
 * @param array $attributes Raw block attributes.
 * @return array Sanitized attributes with snake_case keys.
 */
function buttercup_tag_showcase_sanitize_attributes( $attributes ) {
	...
	if ( ! $enable_multi_tag && count( $tag_slugs ) > 1 ) {
		$tag_slugs = array( reset( $tag_slugs ) );
	}

	$tag_match = strtolower( (string) $attrs['tagMatch'] );
	if ( ! in_array( $tag_match, array( 'any', 'all' ), true ) ) {
		$tag_match = 'any';
	}
	if ( ! $enable_multi_tag ) {
		$tag_match = 'any';
	...
	if ( empty( $post_types ) ) {
		$fallback   = array_values( array_intersect( array( 'post', 'page' ), $valid_post_types ) );
		$post_types = ! empty( $fallback ) ? $fallback : $valid_post_types;
	}

	$order_by = strtolower( (string) $attrs['orderBy'] );
	if ( ! in_array( $order_by, array( 'date', 'title', 'modified' ), true ) ) {
		$order_by = 'date';
	}
	...
	if ( ! in_array( $click_mode, array( 'card-cta', 'cta-only', 'card-only' ), true ) ) {
		$click_mode = 'card-cta';
	}

	$button_style = strtolower( (string) $attrs['buttonStyle'] );
	if ( ! in_array( $button_style, array( 'solid', 'outline', 'text' ), true ) ) {
		$button_style = 'solid';
	}
	...
	if ( ! in_array( $image_aspect_ratio, array( '16/9', '4/3', '3/2', '1/1', '2/3', 'auto' ), true ) ) {
...
/**
 * Build WP_Query arguments for the tag showcase query.
 *
 * @param array $attrs      Sanitized block attributes.
 * @param bool  $for_status Whether this is a status-only query (limits to 1 post).
 * @return array WP_Query arguments.
 */
function buttercup_tag_showcase_get_query_args( $attrs, $for_status = false ) {
	$tax_query = array();
	if ( ! empty( $attrs['tag_slugs'] ) ) {
		if ( 'all' === $attrs['tag_match'] && count( $attrs['tag_slugs'] ) > 1 ) {
...
/**
 * Get a text snippet for a post, from excerpt or content.
 *
 * @param WP_Post $post          The post object.
 * @param int     $snippet_words Number of words for the snippet.
 * @return string Trimmed snippet text.
 */
...
/**
 * Build the inline style attribute string for the showcase wrapper.
 *
 * @param array $attrs Sanitized block attributes.
 * @return string CSS custom properties string.
 */
function buttercup_tag_showcase_build_style_attr( $attrs ) {
	$aspect_ratio = 'auto' === $attrs['image_aspect_ratio']
		? 'auto'
...
/**
 * Build target and rel attributes for links that may open in a new tab.
 *
 * @param bool $open_in_new_tab Whether links should open in a new tab.
 * @return string HTML attributes string or empty string.
 */
...
/**
 * Render the tag showcase block on the front end.
 *
 * @param array $attributes Raw block attributes.
...
function buttercup_render_tag_showcase( $attributes ) {
	...
	$render_cache_key = buttercup_build_cache_key( 'tag_showcase_render', array( $attrs, $current_post_id ) );
	...
	while ( $query->have_posts() ) {
		...
		$card_classes = array( 'buttercup-tag-showcase__card', 'is-click-' . $attrs['click_mode'] );

		$html .= '<article class="' . esc_attr( implode( ' ', $card_classes ) ) . '" data-post-id="' . esc_attr( $post_id ) . '">';

		if ( $has_link && 'cta-only' !== $attrs['click_mode'] ) {
		...
		if ( $attrs['show_thumbnail'] && has_post_thumbnail( $post ) ) {
			$thumb_classes = array( 'buttercup-tag-showcase__thumb' );
			if ( 'auto' === $attrs['image_aspect_ratio'] ) {
		...
		if ( $attrs['show_button'] ) {
			if ( 'card-only' === $attrs['click_mode'] || ! $has_link ) {
...
/**
 * Get the total matching post count for a tag showcase configuration.
 *
 * @param array $attrs Sanitized block attributes.
...
function buttercup_tag_showcase_get_status_count( $attrs ) {
	$current_post_id = $attrs['exclude_current_post'] ? buttercup_tag_showcase_get_current_post_id() : 0;
	$cache_key       = buttercup_build_cache_key( 'tag_showcase_status', array( $attrs, $current_post_id ) );
...
/**
 * REST API callback for the tag showcase status endpoint.
 *
 * @param WP_REST_Request $request REST request object.
 * @return WP_REST_Response Response with count and hasResults.
 */
function buttercup_rest_tag_showcase_status( $request ) {
	$raw_tag_slugs  = isset( $request['tagSlugs'] ) ? explode( ',', (string) $request['tagSlugs'] ) : array();
	$raw_post_types = isset( $request['postTypes'] ) ? explode( ',', (string) $request['postTypes'] ) : array();
