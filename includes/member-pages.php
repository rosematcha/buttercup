<?php
/**
 * Member pages routing, rendering, and virtual page handling.
 *
 * @package Buttercup
 */
...
/**
 * Register rewrite rules for member page routes.
 *
 * @return void
 */
function buttercup_register_member_routes() {
	add_rewrite_tag( '%buttercup_member%', '([a-z0-9-]+)' );
	$bases = get_option( 'buttercup_member_bases', array() );
	if ( ! is_array( $bases ) ) {
		return;
	}
...
/**
 * Check whether member bases have changed between two values.
 *
 * @param array $next New bases value.
 * @param array $prev Previous bases value.
 * @return bool True if the bases have changed.
 */
...
/**
 * Check whether any blocks in the tree have member pages enabled.
 *
 * @param array $blocks       Parsed blocks array.
 * @param array $visited_refs Already-visited reusable block refs.
 * @param int   $depth        Current recursion depth.
 * @return bool True if a team block with member pages is found.
 */
function buttercup_blocks_have_member_pages( $blocks, $visited_refs = array(), $depth = 0 ) {
	...
	foreach ( $blocks as $block ) {
		...
		if ( 'buttercup/team' === ( $block['blockName'] ?? '' ) ) {
...
/**
 * Refresh the stored member bases by scanning all published pages.
 *
 * @return void
 */
function buttercup_refresh_member_bases() {
	...
	foreach ( $pages as $page ) {
		if ( false === strpos( $page->post_content, 'buttercup/team' ) ) {
			continue;
		}
	...
	$prev = get_option( 'buttercup_member_bases', array() );
...
/**
 * Update the member base entry for a single page.
 *
...
/**
 * Handle page save to update member bases.
 *
...
/**
 * Refresh member bases on admin init if not yet populated.
 *
 * @return void
 */
function buttercup_admin_refresh_member_bases() {
	...
	if ( null === get_option( 'buttercup_member_bases', null ) ) {
...
/**
 * Get the cleaned request path relative to the home URL.
 *
...
/**
 * Check whether a page has member pages enabled via its blocks.
 *
 * @param int $page_id The page post ID.
 * @return bool True if the page has member pages.
 */
...
/**
 * Match a request path to a member page route.
 *
 * @param string $request_path The URL path to match.
 * @return array|null Match array with page_id and slug, or null.
 */
...
/**
 * Parse a member request from query vars or cached match.
 *
...
/**
 * Disable canonical redirect for member page URLs.
 *
...
/**
 * Refresh member bases and flush rewrite rules on activation.
 *
 * @return void
 */
...
/**
 * Register the buttercup_member query variable.
 *
...
/**
 * Clean a member name by stripping tags and collapsing whitespace.
 *
...
/**
 * Build unique slugs for each member entry on a page.
 *
 * @param array $entries The member entries array.
 * @return array Entries with slug values assigned.
 */
function buttercup_build_member_slugs_for_page( $entries ) {
	...
	foreach ( $entries as $index => $entry ) {
		$member = $entry['member'] ?? array();
		$name   = buttercup_clean_name( $member['name'] ?? '' );
		$tokens = '' === $name ? array() : preg_split( '/\s+/', $name );
		$first  = $tokens[0] ?? '';
...
/**
 * Collect member entries from parsed blocks recursively.
 *
 * @param array $blocks       Parsed blocks array.
 * @param array $entries      Accumulated entries (passed by reference).
 * @param array $visited_refs Already-visited reusable block refs.
 * @param int   $depth        Current recursion depth.
 * @return array The accumulated entries.
 */
function buttercup_collect_member_entries( $blocks, &$entries, $visited_refs = array(), $depth = 0 ) {
	if ( $depth > BUTTERCUP_MAX_BLOCK_DEPTH ) {
		return $entries;
	}
		...
		if ( 'buttercup/team' === ( $block['blockName'] ?? '' ) ) {
			...
			if ( $enabled ) {
				foreach ( $block['innerBlocks'] ?? array() as $inner ) {
					if ( 'buttercup/team-member' === ( $inner['blockName'] ?? '' ) ) {
						$member_attrs         = $inner['attrs'] ?? array();
...
/**
 * Find a member entry matching a slug within parsed blocks.
 *
 * @param array  $blocks Parsed blocks array.
 * @param string $slug   The member slug to find.
...
/**
 * Build an HTML contact link for a member page.
 *
 * @param string $label Display label for the link.
 * @param string $href  The link URL.
 * @param string $text  Optional display text (defaults to href).
 * @return string The contact link HTML.
 */
...
/**
 * Enqueue CSS assets for member pages.
 *
 * @return void
 */
...
/**
 * Render the member page template and exit.
 *
 * @return void
 */
function buttercup_render_member_page() {
	...
	if ( $can_debug && isset( $_GET['buttercup_debug'] ) ) {
		...
		$payload = array(
			...
			'page_has_team' => $debug_page ? ( false !== strpos( $debug_page->post_content, 'buttercup/team' ) ) : false,
	...
	if ( $page_id > 0 ) {
		...
		// phpcs:ignore WordPress.WP.GlobalVariablesOverride.Prohibited -- Intentionally setting up global $post for template rendering.
		$post = $page;
		if ( $wp_query ) {
	...
	add_filter(
		...
		function ( $parts ) use ( $member_title ) {
	...
	$social_html  = '';
	$social_links = isset( $member['socialLinks'] ) && is_array( $member['socialLinks'] ) ? $member['socialLinks'] : array();
	foreach ( $social_links as $link ) {
		...
		if ( 'email' === $platform ) {
			$href = 0 === strpos( $url, 'mailto:' ) ? $url : 'mailto:' . $url;
		} else {
			$href = $url;
		}
	...
	$shadow_class = $member_shadow && 'none' !== $member_shadow
	...
	if ( $image_url || $image_id ) {
		...
		if ( 'square-600' === $image_source && $image_url ) {
