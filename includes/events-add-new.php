<?php
/**
 * Add New Event wizard and form handling.
 *
 * @package Buttercup
 */
...
function buttercup_redirect_add_new_event() {
    ...
    // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	if ( ! isset( $_GET['post_type'] ) || 'buttercup_event' !== $_GET['post_type'] ) {
		return;
	}
...
function buttercup_add_event_wizard_render() {
	...
	if ( isset( $_SERVER['REQUEST_METHOD'] ) && 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['buttercup_add_event_nonce'] ) ) {
...
function buttercup_wizard_handle_manual() {
	...
	$post_status   = ( 'publish' === $submit_action ) ? 'publish' : 'draft';
	...
	if ( in_array( $page_mode, array( 'editor', 'standalone' ), true ) ) {
...
function buttercup_wizard_handle_ical() {
	...
	if ( isset( $_FILES['ical_file'] ) && ! empty( $_FILES['ical_file']['tmp_name'] ) && isset( $_FILES['ical_file']['error'] ) && UPLOAD_ERR_OK === $_FILES['ical_file']['error'] ) {
		...
		<form method="post" class="buttercup-wizard__form" id="buttercup-wizard-manual" enctype="multipart/form-data">
			...
			<div class="buttercup-wizard__section">
				...
				<div class="buttercup-wizard__field">
					...
					<input type="text" name="event_location" id="event_location" class="large-text" placeholder="<?php echo esc_attr( $recent_location ? $recent_location : __( 'e.g. 1126 N St Marys St, San Antonio, TX', 'buttercup' ) ); ?>" />
