<?php
/**
 * Edit Event wizard and form handling.
 *
 * @package Buttercup
 */
...
function buttercup_redirect_edit_event() {
	...
	if ( 'buttercup_event' !== get_post_type( $post_id ) ) {
		return;
	}

	if ( 'template' !== buttercup_get_event_page_mode( $post_id ) ) {
		return;
	}
...
function buttercup_edit_event_wizard_render() {
	...
	if ( ! $post || 'buttercup_event' !== $post->post_type ) {
	...
	if ( isset( $_SERVER['REQUEST_METHOD'] ) && 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['buttercup_edit_event_nonce'] ) ) {
		...
		if ( ! $error ) {
			...
			if ( $event_url ) {
				...
				if ( in_array( $url_label, array( 'more_info', 'get_tickets', 'register', 'custom' ), true ) ) {
			...
			if ( in_array( $page_mode, array( 'template', 'editor', 'standalone' ), true ) ) {
				update_post_meta( $post_id, '_buttercup_event_page_mode', $page_mode );
			}
			if ( 'standalone' === $page_mode ) {
			...
			update_post_meta( $post_id, 'buttercup_home_mobile_image_id', $mobile_id ? $mobile_id : '' );
			update_post_meta( $post_id, 'buttercup_home_desktop_image_id', $desktop_id ? $desktop_id : '' );
...
function buttercup_edit_event_wizard_page( $post_id, $error = '' ) {
	...
	$url_label_raw    = get_post_meta( $post_id, '_buttercup_event_url_label', true );
	$url_label        = $url_label_raw ? $url_label_raw : 'more_info';
	...
	<div class="wrap buttercup-wizard">
		...
		<form method="post" class="buttercup-wizard__form" id="buttercup-wizard-edit" enctype="multipart/form-data">
			...
			<div class="buttercup-wizard__section">
				...
				<div class="buttercup-wizard__field" id="url-label-custom-field" style="<?php echo ( $event_url && 'custom' === $url_label ) ? '' : 'display:none;'; ?>">
				...
				<div class="buttercup-wizard__field" id="linked-page-field" style="<?php echo 'standalone' === $page_mode ? '' : 'display:none;'; ?>">
				...
				<div class="buttercup-wizard__field" id="custom-slug-field" style="<?php echo ( 'standalone' === $page_mode && ! $linked_page ) ? '' : 'display:none;'; ?>">
					...
					<div class="buttercup-wizard__field-row">
						<div class="buttercup-wizard__field buttercup-wizard__field--half">
							...
							<div class="buttercup-wizard__image-field" id="mobile-image-field">
								<input type="hidden" name="event_homepage_mobile_id" id="event_homepage_mobile_id" value="<?php echo esc_attr( $mobile_id ? $mobile_id : '' ); ?>" />
						...
						<div class="buttercup-wizard__field buttercup-wizard__field--half">
							...
							<div class="buttercup-wizard__image-field" id="desktop-image-field">
								<input type="hidden" name="event_homepage_desktop_id" id="event_homepage_desktop_id" value="<?php echo esc_attr( $desktop_id ? $desktop_id : '' ); ?>" />
			...
			<div class="buttercup-wizard__actions">
				<?php if ( $is_published ) : ?>
					<?php submit_button( __( 'Update', 'buttercup' ), 'primary large', 'submit', false, array( 'id' => 'buttercup-btn-update' ) ); ?>
				...
				<?php else : ?>
					...
					<?php submit_button( __( 'Publish', 'buttercup' ), 'primary large', 'submit', false, array( 'id' => 'buttercup-btn-publish' ) ); ?>
				<?php endif; ?>
