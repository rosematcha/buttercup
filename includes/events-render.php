<?php
/**
 * Rendering functions for the events block and event cards.
 *
 * @package Buttercup
 */
...
function buttercup_render_events( $attributes, $content, $block ) {
	...
	$cache_key = buttercup_build_cache_key( 'events', array( $display_mode, $events_to_show, $show_past_link ) );
	$cached    = buttercup_cache_get( $cache_key );
	if ( null !== $cached ) {
		return $cached;
	}

	if ( 'past' === $display_mode ) {
 ...
 * @param bool $show_past_link Whether to show a link to past events.
 * @return string Rendered HTML.
 */
