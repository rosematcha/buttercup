<?php
/**
 * Facebook HTML page parser for extracting event data.
 *
 * @package Buttercup
 */
 ...
 * @return array Parsed events array.
 */
function buttercup_fb_extract_jsonld_events( $html ) {
	...
	foreach ( $matches[1] as $json_str ) {
		...
		$items = isset( $data['@type'] ) ? array( $data ) : ( isset( $data[0] ) ? $data : array() );
 ...
 *
 * @param string $html Raw HTML content.
 * @return array Deduplicated parsed events array.
 */
 ...
 * @return array|null Normalized event array or null.
 */
 ...
 *
 * @param DOMNode $node Starting DOM node.
 * @return DOMNode|null The card container node or the parent node.
 */
 ...
 * @return bool True if the text looks like a date.
 */
...
function buttercup_fb_deduplicate_events( $events ) {
	...
	foreach ( $events as $event ) {
		$key = $event['url'] ? $event['url'] : $event['title'];
		if ( isset( $seen[ $key ] ) ) {
			continue;
		}
		$seen[ $key ] = true;
